    ...
    <div>
      ...
      <div className="relative h-[70vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-16">
          ...
          <h1 className="font-serif text-5xl md:text-7xl text-ivory font-light mb-3">{itin.title}</h1>
          <p className="text-ivory/70 text-lg">{itin.subtitle}</p>
        </div>
      </div>

      {/* Quick Facts */}
      <div className="bg-forest text-ivory py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            <div className="flex flex-col items-center gap-2">
              <Clock size={20} className="text-gold" />
              <p className="text-xs tracking-widest uppercase text-ivory/50">Duration</p>
              <p className="font-serif text-lg">{itin.duration} Days</p>
            </div>
            <div className="flex flex-col items-center gap-2">
              <Users size={20} className="text-gold" />
              <p className="text-xs tracking-widest uppercase text-ivory/50">Group Size</p>
              <p className="font-serif text-lg">{itin.groupSize}</p>
            </div>
            <div className="flex flex-col items-center gap-2">
              <Activity size={20} className="text-gold" />
              <p className="text-xs tracking-widest uppercase text-ivory/50">Physical Rating</p>
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-xs tracking-widest uppercase text-ivory/50">Price From</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <FadeInUp>
            ...
            <p className="text-lg text-charcoal-soft leading-relaxed">{itin.overview}</p>
          </FadeInUp>
          <FadeInUp delay={0.1}>
            ...
            <ul className="space-y-3">
              {itin.highlights.map((h) => (
                <li key={h} className="flex items-start gap-3">
                  ...
                  <span className="text-charcoal-soft">{h}</span>
                </li>
              ))}
            </ul>
          </FadeInUp>
      ...
      <div className="bg-ivory-warm py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionLabel className="mb-12">Day By Day</SectionLabel>
          <div className="space-y-8">
            {itin.days.map((day) => (
              <FadeInUp key={day.day} delay={0.05}>
                <div className="flex gap-6">
                  <div className="flex flex-col items-center">
                    <div className="w-10 h-10 rounded-full bg-gold flex items-center justify-center text-forest-dark font-sans font-semibold text-sm flex-shrink-0">
                      {day.day}
                    </div>
                  ...
                  <div className="pb-8 flex-1">
                    ...
                    <h3 className="font-serif text-xl text-forest mb-3">{day.title}</h3>
                    <p className="text-charcoal-soft leading-relaxed">{day.description}</p>
                    {day.image && (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {itin.accommodations.map((acc) => (
              <div key={acc.name} className="border border-ivory-deep overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <h3 className="font-serif text-xl text-forest mb-1">{acc.name}</h3>
                  <p className="text-charcoal-soft text-sm">{acc.location} — {acc.nights} night{acc.nights !== 1 ? "s" : ""}</p>
                </div>
              </div>
            ))}
          </div>
