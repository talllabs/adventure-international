    ...
    <div>
      ...
      <FadeInUp>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <p className="text-lg text-white/60 leading-relaxed">{dest.intro}</p>
      ...
      <div className="bg-[#111111] py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionLabel className="mb-8" light>Signature Experiences</SectionLabel>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            {dest.wildlifeHighlights.map((wl) => (
              <div key={wl.species} className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 border border-white/10 bg-[#111111]">
                <div>
                  <h4 className="font-serif text-lg text-white">{wl.species}</h4>
                </div>
                <div>
                  <p className="text-xs text-gold tracking-widest uppercase mb-1">Best Season</p>
                  <p className="text-white/60 text-sm">{wl.bestSeason}</p>
                </div>
                <div>
                  <p className="text-xs text-white/30 tracking-widest uppercase mb-1">Notes</p>
                  <p className="text-white/60 text-sm">{wl.notes}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {itineraries.map((itin) => itin && (
                <Link key={itin.slug} href={`/itineraries/${itin.slug}`} className="group border border-gold/20 p-8 hover:border-gold transition-colors">
                  <h3 className="font-serif text-2xl text-white group-hover:text-gold transition-colors mb-2">{itin.title}</h3>
                  <p className="text-white/60 text-sm mb-4">{itin.subtitle}</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {dest.lodging.map((lodge) => (
              <div key={lodge.name} className="group overflow-hidden border border-white/10">
                ...
                <div className="p-6">
                  ...
                  <h3 className="font-serif text-xl text-white mb-2">{lodge.name}</h3>
                  <p className="text-white/60 text-sm leading-relaxed">{lodge.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-[#111111] py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="font-serif text-xl text-white">{dest.bestTimeToVisit}</p>
        </div>
      </div>
              ...
              {related.map((r) => r && (
                <Link key={r.slug} href={`/destinations/${r.region}/${r.slug}`} className="group block relative aspect-[3/4] overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    <h3 className="font-serif text-white text-xl">{r.name}</h3>
                    <p className="text-white/60 text-sm">{r.country}</p>
                  </div>
                </Link>
              ))}
            </div>
